...
class GitHubWalkthroughTest < Minitest::Test
  ...
  def test_walkthrough_creates_comment_and_reads_back_the_review_and_head
    github = client(snapshot_response, review_response, review_response, snapshot_response)
    ...
    assert_equal({ 'event' => 'COMMENT', 'commit_id' => HEAD, 'body' => 'A walkthrough.' },
                 JSON.parse(@calls[1].last))
    assert_equal %w[gh api repos/owner/repo/pulls/42/reviews/123 --method GET --input -], @calls[2].first
  end
