...
| Model/effort recommendation and solo default | Avoid paying for expensive reasoning or delegation on bounded work; name an available model and low effort, pause for the maintainer to change host settings, and verify the actual setting on resumption. | L / L; an automatic model-routing service would be H / UNKNOWN until scoped. | Keep the short recommendation and one startup checkpoint. Escalate from observed difficulty; compare total retry/review cost before claiming savings. |
