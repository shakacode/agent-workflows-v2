...
| Cursor | Compatibility target; validate after Claude Code. | Native skills and session/model metadata are documented. Isolated installer accepted `~/.cursor/skills`. One Cursor Desktop 3.20.21 session did not list `aw` from `~/.agents/skills`. Agent-chat discovery after the user-skills link remains unconfirmed. Full V2 delivery and per-request token/effort coverage remain UNKNOWN; do not promise reporting parity from skill compatibility alone. |
