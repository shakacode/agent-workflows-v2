  ...
  class CommentTrustConfig
    ...
    def valid_blob?(object)
      object['__typename'] == 'Blob' && object['isBinary'] == false && object['isTruncated'] == false &&
        object['byteSize'].is_a?(Integer) && object['byteSize'] <= CommentTrustSettings::MAX_BYTES &&
        object['text'].is_a?(String)
    end

    def settings
      @settings ||= CommentTrustSettings.new(@github.repository)
    end
